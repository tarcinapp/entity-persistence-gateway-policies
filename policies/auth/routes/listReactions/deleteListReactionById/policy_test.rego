...
test_allow_to_admin if {
	...
	allow with input as produce_input_doc_by_role(["tarcinapp.listReactions.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.reactions.delete.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.listReactions.delete.admin"])
}
...
produce_input_doc_by_role(roles) := test_body if {
	test_body = {
		...
		"httpMethod": "DELETE",
		"requestPath": "/list-reactions/123",
		"queryParams": {},
		...
		"originalRecord": {"_id": "123", "_kind": "listReactionKind"},
	}
}
