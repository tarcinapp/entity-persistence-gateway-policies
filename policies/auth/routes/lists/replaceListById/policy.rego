...
effective_role := role if {
	role := dispatcher.get_effective_role("lists", "update", input.originalRecord)
}
...
forbidden_find_list := res if {
	effective_role != null
	res := central_policy.get_forbidden_fields_for_role("lists", "find", input.originalRecord, effective_role)
}
...
forbidden_update_list := res if {
	effective_role != null
	res := central_policy.get_forbidden_fields_for_role("lists", "update", input.originalRecord, effective_role)
}
