package policies.auth.routes.lists.deleteListById.policy
...
test_allow_to_admin if {
	...
	allow with input as produce_input_doc_by_role(["tarcinapp.lists.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.records.delete.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.lists.delete.admin"])
}
