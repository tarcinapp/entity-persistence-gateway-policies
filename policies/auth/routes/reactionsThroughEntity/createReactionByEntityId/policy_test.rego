...
# Combined tests for createReactionByEntityId: role-based creation + source visibility
...
test_allow_admin_create_reaction if {
	allow with input as produce_input_doc_by_role_and_source("tarcinapp.admin", true, {"_kind": "like"}, "private", "2020-01-01T00:00:00Z", null)
}

test_allow_editor_create_reaction if {
	allow with input as produce_input_doc_by_role_and_source("tarcinapp.editor", true, {"_kind": "like"}, "protected", "2020-01-01T00:00:00Z", null)
}

# Members allowed only if they can see the source (based on findEntityById rules)
test_allow_member_when_source_public_active if {
	allow with input as produce_input_doc_by_role_and_source("tarcinapp.member", true, {"_kind": "love"}, "public", "2020-01-01T00:00:00Z", null)
}

test_not_allow_member_when_source_protected_not_member if {
	not allow with input as produce_input_doc_by_role_and_source("tarcinapp.member", true, {"_kind": "love"}, "protected", "2020-01-01T00:00:00Z", null)
}

# Member who is direct owner can react to pending or active but not passive
test_not_allow_member_owner_pending if {
	not allow with input as produce_input_doc_by_role_and_source_with_owner("tarcinapp.member", true, {"_kind": "haha"}, "private", null, null, ["ebe92b0c-bda2-49d0-99d0-feb538aa7db6"], [])
}

test_not_allow_member_owner_inactive_passive if {
	not allow with input as produce_input_doc_by_role_and_source_with_owner("tarcinapp.member", true, {"_kind": "haha"}, "public", "2020-01-01T00:00:00Z", "2021-01-01T00:00:00Z", ["ebe92b0c-bda2-49d0-99d0-feb538aa7db6"], [])
}
...
test_not_allow_visitor_even_if_source_public_active if {
	not allow with input as produce_input_doc_by_role_and_source("tarcinapp.visitor", true, {"_kind": "like"}, "public", "2020-01-01T00:00:00Z", null)
}
...
test_allow_member_with_ownerGroups_valid if {
	allow with input as produce_input_doc_by_role_and_source_with_payload_groups("tarcinapp.member", true, {"_kind": "wow", "_ownerGroups": ["group-1"]}, "public", "2020-01-01T00:00:00Z", null)
}

test_not_allow_member_with_ownerGroups_invalid if {
	not allow with input as produce_input_doc_by_role_and_source_with_payload_groups("tarcinapp.member", true, {"_kind": "wow", "_ownerGroups": ["group-2"]}, "public", "2020-01-01T00:00:00Z", null)
}
...
produce_input_doc_by_role_and_source_with_payload_groups(roles, is_email_verified, requestPayload, visibility, validFrom, validUntil) := test_body if {
	test_body = {
		...
		"queryParams": {},
		"encodedJwt": test.produce_token({
			"sub": "ebe92b0c-bda2-49d0-99d0-99d0-feb538aa7db6",
			"name": "John Doe",
			"admin": true,
			"iat": 1516239022,
			"email_verified": is_email_verified,
