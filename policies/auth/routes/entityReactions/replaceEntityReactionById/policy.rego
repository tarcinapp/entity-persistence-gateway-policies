...
effective_role := role if {
	role := dispatcher.get_effective_role("entityReactions", "update", input.originalRecord)
}
...
forbidden_find_list := res if {
	effective_role != null
	res := central_policy.get_forbidden_fields_for_role("entityReactions", "find", input.originalRecord, effective_role)
}
...
forbidden_update_list := res if {
	effective_role != null
	res := central_policy.get_forbidden_fields_for_role("entityReactions", "update", input.originalRecord, effective_role)
}
...
# check if there are any forbidden fields with different values or removals
...
can_see_related_entity if {
	...
	# Admin
	entity_role_utils.is_user_admin("find", input.originalRecord._relationMetadata)
	verification.is_email_verified
}
...
can_see_related_entity if {
	...
	# Editor
	entity_role_utils.is_user_editor("find", input.originalRecord._relationMetadata)
	verification.is_email_verified
}
...
can_see_related_entity if {
	...
	# Member
	entity_role_utils.is_user_member("find", input.originalRecord._relationMetadata)
	verification.is_email_verified
...
can_see_related_entity if {
	...
	# Visitor
	entity_role_utils.is_user_visitor("find", input.originalRecord._relationMetadata)
	verification.is_email_verified
