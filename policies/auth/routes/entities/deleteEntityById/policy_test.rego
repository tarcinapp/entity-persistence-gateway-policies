package policies.auth.routes.entities.deleteEntityById.policy
...
test_allow_to_admin if {
	...
	allow with input as produce_input_doc_by_role(["tarcinapp.entities.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.records.delete.admin"])

	allow with input as produce_input_doc_by_role(["tarcinapp.entities.delete.admin"])
}
...
produce_input_doc_by_role(roles) := test_body if {
	test_body = {
		...
		"originalRecord": {"_id": "123", "_kind": "entityKind"},
	}
}
