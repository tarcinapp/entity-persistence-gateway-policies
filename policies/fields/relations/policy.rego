...
import data.policies.fields.relations.policy.forbiddenFields as forbiddenFields
...
can_user_find_field(fieldName) if {
	...
	pattern := sprintf(`%s\.relations\.fields\.%s\.(find|update|create|manage)`, [input.appShortcode, fieldName])
...
can_user_create_field(fieldName) if {
	...
	token.payload.roles
	role := token.payload.roles[_]
	pattern := sprintf(`%s\.relations\.fields\.%s\.(create|manage)`, [input.appShortcode, fieldName])
	regex.match(pattern, role)
}
...
can_user_update_field(fieldName) if {
	...
	token.payload.roles
	role := token.payload.roles[_]
	pattern := sprintf(`%s\.relations\.fields\.%s\.(update|manage)`, [input.appShortcode, fieldName])
	regex.match(pattern, role)
}
